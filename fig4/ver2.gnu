...
plot [:5][:] for [r in replicas] \
    sprintf("< cat seed_*_displacement_spectra_replica_%s.dat | sort -n -k 1", r) u 2:($2**2*$3*512) smooth unique w lp t sprintf('sim %s', r), \
...
plot for [r in replicas] \
    sprintf("< cat seed_*_correlation_replica_%s.dat | sort -n -k 1", r) u 1:2 smooth unique w lp t sprintf('sim %s', r), \
